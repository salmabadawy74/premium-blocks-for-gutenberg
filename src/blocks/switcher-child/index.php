...
function get_premium_switcher_child_css( $attributes, $unique_id ) {
	...
	// 		$container_grid2 = '';
	// 		$container_bg    = '';
	// 		if ( isset($background['backgroundType']) && $background['backgroundType'] && 'gradient' === $background['backgroundType'] ) {
 ...
 */
function render_block_premium_switcher_child( $attributes, $content, $block ) {
print_r($attributes);
	...
	// 		$css = get_premium_switcher_child_css( $attributes, $id );
	// 	if ( ! empty( $css ) ) {
	// 		$block_helpers = pbg_blocks_helper();
 ...
 *
 * @uses render_block_premium_switcher_child()
 * @throws WP_Error An WP_Error exception parsing the block definition.
 */
function register_block_premium_switcher_child() {
	register_block_type_from_metadata(
		PREMIUM_BLOCKS_PATH . 'src/blocks/switcher-child/index.js',
		array(
			'render_callback' => 'render_block_premium_switcher_child',
		)
	);
}
add_action( 'init', 'register_block_premium_switcher_child' );
