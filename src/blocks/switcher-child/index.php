...
function get_switcher_child_css_style( $attributes, $unique_id ) {
...
function should_render_inline_switcher( $name, $unique_id ) {
 ...
 function render_block_pbg_switcher_child( $attributes, $content ) {
	...
	if ( ! wp_style_is( $style_id, 'enqueued' ) && apply_filters( 'Premium_BLocks_blocks_render_inline_css', true, 'content-switcher', $unique_id ) ) {
		...
		$css = get_switcher_child_css_style( $attributes, $unique_id );
		if ( ! empty( $css ) ) {
			$block_helpers = new pbg_blocks_helper();
			if ( should_render_inline_switcher( 'content-switcher', $unique_id ) ) {
...
function register_block_pbg_switcher_child() {
    register_block_type_from_metadata(
		...
		array(
            'render_callback' => 'render_block_pbg_switcher_child',
            )
        );
}
...
function switcher_child_enqueue() {
	wp_register_script(
		'gpb-switcher-child-block-script',
			PREMIUM_BLOCKS_URL . 'src/blocks/switcher-child/switcher-child.js',
			array( 'jquery' ),
...
add_action( 'enqueue_block_assets', 'switcher_child_enqueue' );