...
const { BlockControls, InspectorControls, AlignmentToolbar, ColorPalette, RichText, MediaUpload } = wp.editor;

const IMG_AlLIGNMENT = [
  {
      ...
      <div>
        <BlockControls key="controls">
          <AlignmentToolbar
            ...
            value = {imgAlign}
            onChange = {newAlign => setAttributes({ imgAlign: newAlign || "center" })}
            alignmentControls = {IMG_AlLIGNMENT}
          />
        </BlockControls>
        <InspectorControls key={"inspector"}>
          ...
          >
            ...
            <div className="premium-control-toggle">
              ...
              <RangeControl
                ...
                value={imageWidth}
                max="600"
                onChange={newValue => setAttributes({ imageWidth: newValue || "1" })}
                initialPosition={50}
                min={1}
                max={100}
              />
          ...
          >
            <PremiumFilters
              ...
              hue={hue}
              onChangeBlur={newValue => setAttributes({ blur: newValue || "0" })}
              onChangeBright={newValue => setAttributes({ bright: newValue || "100" })}
              onChangeContrast={newValue => setAttributes({ contrast: newValue || "100" })}
              onChangeSat={newValue => setAttributes({ saturation: newValue || "100" })}
              onChangeHue={newValue => setAttributes({ hue: newValue || "0" })}
            />
          </PanelBody>
