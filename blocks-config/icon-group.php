...
function get_premium_icon_group_css( $attr, $unique_id ) {
	...
	if ( isset( $attr['groupIconBorder'] ) ) {
		...
		$css->set_selector( '.' . $unique_id . ' .premium-icon-container > .premium-icon__link .premium-icon-hover i');
...
function render_block_pbg_icon_group( $attributes, $content, $block ) {
	...
	wp_enqueue_style(
		'pbg-icon-group-style',
		PREMIUM_BLOCKS_URL . 'assets/css/minified/icon-group.min.css',
		array(),
