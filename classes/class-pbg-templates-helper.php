...
class PBG_Template_Helper {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_pbg-block-templates', array( $this, 'ajax_import_category' ) );

        // add_action( 'admin_head', array( $this, 'get_cached_data' ) );
    ...
    public function get_cached_data() {
        ...
        foreach ( $list_files as $key => $file_name ) {
            if ( file_exists( $dir . '/' . $file_name . '.json' ) ) {
                ...
                $response = get_transient('pbg_get_template');
                if ( false == $response ) {
                    $this->get_remote_templates();
                }
                $expires = (int) get_option( '_transient_timeout_pbg_get_template', 0 );
